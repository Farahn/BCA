...
X_train, y_train, X_val, y_val, doc_vocab_size, embedding = read_test_train(glove_vocab, embedding_dict, embedding_dim, SEQUENCE_LEN = SEQUENCE_LENGTH, SEQUENCE_LEN_D = SEQUENCE_LENGTH_D, dname =  'han%d'%(HIDDEN_SIZE + HIDDEN_SIZE_D), tr = train_split, max_vocab = max_vocab)
...
if __name__ == "__main__":
    ...
    with tf.Session(config = config) as sess:
        ...
        saver.save(sess, "model/model%d" %(HIDDEN_SIZE + HIDDEN_SIZE_D))
        print("saved at model/model%d" %(HIDDEN_SIZE + HIDDEN_SIZE_D))
